...
- Share card: `assets/og.png` (1200×630) is the Open Graph image, generated by `scripts/generar-og.sh`. The design lives inside the script — text is converted to outlines from the woff2 subsets in `assets/fonts/`, so rsvg doesn't depend on installed fonts. The subsets are variable (`wght` 100..700) and default to Thin, so they must be instanced at the real weight. The script checks the size, that there's no alpha, that the lockup fits the centre 630 square (WhatsApp crops narrow previews) and the 80 KB budget. Scrapers cache by URL — a redesign that must show up immediately needs a new filename plus updated `og:image`.
